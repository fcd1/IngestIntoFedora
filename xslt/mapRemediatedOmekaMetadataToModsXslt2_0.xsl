...
<xsl:stylesheet version="2.0"
  ...
  <!-- fcd1, 08/20/14: Renamed to DigitalOriginOld because gonna try -->
  <!-- to use regular expression (XSLT 2.0) in new version -->
  <xsl:template name="DigitalOriginOld">
  ...
  <!-- fcd1, 08/20/14: MODS <digitalOrigin>, uses XSLT 2.0 -->
  <xsl:template name="DigitalOrigin">
    <xsl:for-each select='*[matches(name(),"item_-_MODS_-_DigitalOrigin")]'>
      <xsl:if test=" . != '' ">
	<digitalOrigin><xsl:value-of select="."/></digitalOrigin>
      </xsl:if>
    </xsl:for-each>
